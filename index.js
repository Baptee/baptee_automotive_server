...
const uri = `mongodb+srv://baptee:baptee123@cluster0.xkmayyi.mongodb.net/?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true, serverApi: ServerApiVersion.v1 });

client.connect(err => {
  const userCollection = client.db("BapteeAutomative").collection("Users");
  const mechanicCollection = client.db("BapteeAutomative").collection("Mechanics");
  const orderCollection = client.db("BapteeAutomative").collection("Orders");
...
app.listen(process.env.PORT || port, () => {
  console.log(`Baptee Automotive Backend Running on port ${port}`)
})